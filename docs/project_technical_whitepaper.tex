...
\title{\bfseries VLN--CogNav Object Navigation 系统技术白皮书}
\author{Huawei Nav / VLN 工程实现}
\date{\today}
...
VLN 的基本思想是将环境组织为 Room--Viewpoint--Object 三层结构，从而让 VLM
...
\begin{figure}[htbp]
  \centering
  \IfFileExists{assets/vln_navigation_pipeline.png}{%
    \includegraphics[width=\textwidth]{assets/vln_navigation_pipeline.png}%
  }{%
    \fbox{\parbox{0.88\textwidth}{\centering
      项目管线图素材未随当前代码版本发布。请将正式图稿放入
      \texttt{docs/assets/vln_navigation_pipeline.png} 后重新编译。}}
  }
  \caption{VLN--CogNav Object Navigation 系统管线示意图。}
  \label{fig:pipeline}
\end{figure}
...
\begin{lstlisting}[style=pseudo,caption={ConceptQuery 的核心字段}]
ConceptQuery:
...
约束被表示为声明式结构。parser 只声明约束，不验证约束；运行时再根据
\texttt{verifier} 字段选择 planner、geometry、VLM 或 metadata 处理。

\begin{lstlisting}[style=pseudo,caption={Constraint 的核心字段}]
Constraint:
...
没有 frontier 的区域则取连通区域中心。这样既保留了 VLN 的稀疏拓扑思想，
又避免将每个可通行网格都暴露给 VLM。
...
\begin{lstlisting}[style=pseudo,caption={VerificationResult 的核心字段}]
VerificationResult:
...
视角优化继承 VLN 中 Viewpoint-Optimized Re-Verification 的思想：目标第一次被
...
\begin{lstlisting}[style=pseudo,caption={Final verifier 输出字段说明}]
...
Output fields:
  ...
  confidence                   # 关系判断置信度
  need_better_view             # 证据不足时请求更好视角
  reason                       # 写入 DynamicSemanticEdge / RelationLedger 的解释
\end{lstlisting}
...
\subsection{模块分层与运行职责}

实物模式采用“SysNav 管底层闭环，VLN 管语义决策”的分层。该分层不是简单的
ROS topic 转发，而是将对象身份、检测词表、运动状态和证据采集拆成可替换的
contract。更换相机、检测器或底盘控制器时，原则上不应改 instruction parser、
concept grounding 和 final verifier。

\begin{longtable}{p{0.18\linewidth}p{0.24\linewidth}p{0.24\linewidth}p{0.26\linewidth}}
\toprule
\textbf{层级} & \textbf{当前实现} & \textbf{输出} & \textbf{失败/降级语义} \\
\midrule
Contract layer &
\texttt{real\_robot/contracts.py} &
\texttt{RealObservation}, \texttt{SemanticMapSnapshot}, \texttt{NavigationIntent},
\texttt{MotionGoal}, \texttt{ViewEvidence} &
不推断语义；字段不完整时保留 \texttt{UNKNOWN} 或 \texttt{None}。\\
Vocabulary layer &
\texttt{real\_robot/detector\_vocabulary.py} &
label provenance 与 grounding context &
不写隐藏 alias；词表差异交给 grounding prompt。\\
ROS adapter layer &
\texttt{real\_robot/sysnav\_ros\_adapters.py} &
\texttt{DetectionFrame}, \texttt{ObjectNodeSnapshot}, \texttt{RoomSnapshot},
\texttt{/way\_point} &
只做字段适配；\texttt{STOP}/\texttt{WAIT} 不发布 waypoint。\\
Semantic-map bridge &
\texttt{SysNavSemanticMapBridge} &
\texttt{SemanticMapSnapshot} &
未收到对象图时返回 \texttt{None}，高层进入 \texttt{WAIT}。\\
Instruction runtime &
\texttt{SysNavInstructionRuntime} &
\texttt{NavigationIntent -> MotionGoal} &
策略无法决策时输出 \texttt{WAIT} 或探索意图。\\
Motion bridge &
\texttt{RosWaypointController} &
\texttt{/way\_point} 与 \texttt{NavigationStatus} &
可接入真实 status provider；adapter 本身不猜测到达。\\
Evidence loop &
\texttt{ViewpointEvidenceLoop} &
\texttt{ViewEvidence} 与 verifier decision &
只有 \texttt{REACHED} 后采集证据；\texttt{BLOCKED}/\texttt{TIMEOUT} 回到策略层。\\
ROS overlay &
\texttt{real\_robot/ros2\_ws} &
SysNav-compatible semantic topics &
第一版复用 SysNav 节点，后续 detector 可替换为兼容 publisher。\\
\bottomrule
\end{longtable}

\begin{lstlisting}[style=pseudo,caption={Real-robot runtime dataflow}]
Robot sensor topics
...
这一设计的关键约束是：adapter 层不能替上层做语义判断。\texttt{RosObjectNodeAdapter}
可以把 SysNav 的 \texttt{ObjectNode.label="cabinet"} 转成
\texttt{ObjectNodeSnapshot(label="cabinet")}，但不能在这里判断它是否满足
``shelf anchor''。这种判断必须由 \texttt{ConceptQuery}、\texttt{DetectorVocabulary}
和视觉证据共同进入 grounding/verifier。类似地，\texttt{RosRoomNodeAdapter}
不把 room node 写成 bedroom 或 living room；SysNav 的 room 几何节点只作为空间区域，
语义房间名由 VLN 的 room policy 或 VLM 推断。

\subsection{对象身份、缓存与检测词表}

实物系统中 detector track id、分割 mask 和对象融合结果都会随时间变化。因此
VLN 不应直接用 bbox 或 label 作为长期身份。当前实现采用如下优先级：

\begin{lstlisting}[style=pseudo,caption={Object identity priority}]
Object identity key:
...
检测词表通过 \texttt{DetectorVocabularyAdapter} 读入 SysNav \texttt{objects.yaml}。
它输出的不是同义词表，而是 provenance：

\begin{lstlisting}[style=pseudo,caption={Detector vocabulary provenance}]
DetectorVocabulary:
  labels                 # detector 实际可输出的标签集合
  prompt_terms           # detector prompt 或 objects.yaml 中的候选词
  source                 # 词表来源，如 sysnav_objects_yaml
  metadata               # 模型、配置路径、加载时间等附加信息

LabelProvenance:
  raw_label              # SysNav/检测器原始标签
  normalized_label       # 仅做格式规范化，不表达语义等价
  in_detector_vocab      # 该标签是否来自当前 detector 词表
  detector_source        # 便于回放时追踪标签来源
\end{lstlisting}

这些字段进入 concept grounding prompt。换言之，\texttt{book/books/shelf/bookcase}
这类泛化仍由 prompt-first grounding 和证据验证处理，而不是在 ROS adapter 中静默写规则。

\subsection{运动闭环与证据闭环的边界}

实物模式中，\texttt{NavigationIntent} 是高层语义决策，\texttt{MotionGoal} 是底层
可执行请求。二者不能合并，否则 final verifier 很容易越权决定底盘状态。

\begin{lstlisting}[style=pseudo,caption={Motion and evidence boundary}]
NavigationIntent:
...
如果 \texttt{NavigationStatus} 是 \texttt{BLOCKED} 或 \texttt{TIMEOUT}，
\texttt{ViewpointEvidenceLoop} 不会调用 \texttt{EvidenceProvider.capture()}。
这条约束使语义验证和真实运动执行保持一致：VLM 可以评价当前证据是否满足任务，
但不能把一个没有到达的 waypoint 解释成成功。
...
生命周期和房间节点；VLN 订阅只读 snapshot，做语义决策后发布 \texttt{/way\_point}。
...
后续迁移到具体机器人时，需要补齐或替换的不是 VLN 高层策略，而是真实
