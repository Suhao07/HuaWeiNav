...
flowchart TB
  ...
  subgraph Sim[Habitat Simulator Runtime]
...
```python
InstructionPlan:
    raw_instruction: str          # 原始指令；final verifier 必须回到这句话做最终确认
    dataset_target: str           # 数据集或 benchmark 给出的结构化目标，可为空
    task_type: str                # 任务类型，如 object_goal / relation_goal / counting
    eval_mode: str                # 成功模式，如任一目标成功、顺序完成或计数完成
    targets: list[TargetQuery]    # 可终止任务的目标集合，通常来自 terminal concepts
    constraints: list[Constraint] # 房间、属性、空间关系、数量、顺序等显式约束
    search_priors: SearchPriors   # 搜索先验，只影响探索顺序，不直接决定成功
    execution: ExecutionPolicy    # 执行策略，如 anchor-first、sequence、count
    concept_queries: list[ConceptQuery] # 所有需要 grounding 的目标/anchor/support 概念
    valid: bool                   # 解析是否可执行；false 时应进入 fallback 或报错
    diagnostics: dict             # 解析来源、LLM 输出、fallback 和调试信息
```
...
```python
ConceptQuery:
    id: str                       # plan 内稳定概念 ID，用于缓存和 ledger 索引
    name: str                     # 指令中的规范概念名，如 book / shelf / red chair
    role: primary | anchor | support | secondary
                                  # primary 是主要目标；anchor/support 只辅助搜索和验证
    detector_terms: list[str]     # 传给检测器或匹配器的候选词，由 grounding 层生成
    aliases: list[str]            # 语义等价或近似表达，供 runtime concept matcher 使用
    description: str              # 给 LLM/VLM 的概念描述，用于开放词表和小目标泛化
    negative_terms: list[str]     # 容易混淆但不应匹配的概念，如 shelf 不能匹配 book
    terminal: bool                # 是否允许触发最终成功；anchor/support 必须为 false
```
...
Constraint:
    type: room | spatial | sequence | count | attribute | area | co_occurrence
                                  # 约束类型，决定运行时由哪个 evaluator 处理
    subject: str                  # 约束主体，一般是 terminal concept 或候选实例
    relation: str                 # 关系谓词，如 in / on / near / before / count>=N
    object: str                   # 约束客体，如 room、anchor concept 或数量目标
    hardness: hard | soft         # hard 必须满足；soft 只作为搜索排序或提示
    verifier: planner | geometry | vlm | metadata
                                  # 验证来源：规划器、几何模块、VLM 或数据集 metadata
```
...
Concept:
  name: shelf                         # 待 grounding 的指令概念
  role: anchor                        # 该概念只作为参考物，不允许触发最终 STOP
  instruction: find a book on a shelf # 原始指令上下文，帮助判断 shelf 的任务角色

Available detector classes:
  [book, cabinet, bookshelf, table, chair, wall, ...] # 当前检测器可用词表
...
```text
detector_terms: 传给检测器或候选预筛的词；来自本次 grounding，不是代码硬编码。
aliases: 运行时 concept matcher 可参考的语义近似表达。
description: 给 LLM/VLM 的自然语言概念定义，用于开放词表泛化。
negative_terms: 易混淆但不应匹配的概念。
terminal: 是否允许该 concept 触发最终成功；anchor 必须为 false。
```
...
```python
Observation:
    rgb: H x W x 3      # 当前视角 RGB 图像，是检测与 VLM 证据的主要输入
    depth: H x W x 1    # 深度图；实机可由 RGB-D 或 LiDAR 投影得到
    pose: SE3           # 相机/机器人位姿，用于将局部观测投影到全局地图
```
...
RGB image + text prompts
  -> GroundingDINO boxes             # 开放词表检测，给出候选 bbox
  -> SAM masks                       # 对 bbox 内实例进行像素级分割
  -> mask + depth back-projection    # 利用 depth/pose 将 2D mask 转成 3D 点
  -> object point clouds             # 形成 object node 的几何证据
```
...
scene point cloud + navigable point cloud
  ...
  -> connected components on the remaining free space
     # 用连通组件作为当前工程实现的 room region
  -> assign navigation nodes to connected room regions
     # 将 viewpoint/topology node 归属到对应 room
...
terminal concept: book
  # 最终要找到并确认的目标概念，可以触发成功
anchor concept: shelf
  # 只作为搜索参考和关系验证对象，不允许直接 STOP
relation: on
  # 需要运行时验证的动态 object-object 关系
...
```text
candidate object
  + original instruction      # 原始自然语言，最终确认必须回到用户真实意图
  + InstructionPlan           # 编译后的目标、角色、约束和执行策略
  + geometry facts            # 距离、bbox、可达性、视角质量等硬证据
  + relation edges            # 已验证或待验证的动态语义关系
  + view-control history      # 已尝试视角、best evidence、预算状态
  -> VerificationResult       # 结构化停止判定结果
```
...
```python
VerificationResult:
    satisfied: bool           # 原始任务是否整体满足；accept 时必须为 true
    decision: accept | reject_candidate | need_better_view | need_relation_check | uncertain
                              # 控制器下一步动作：停止、拒绝候选、换视角、补关系验证或继续观察
    semantic_satisfied: bool  # 候选语义是否符合原始指令，不等同于视角足够
    view_sufficient_for_stop: bool
                              # 当前视角是否足以作为最终停止证据
    hard_constraints: dict    # 距离、可达性、关系必满足项等结构化硬约束
    confidence: float         # verifier 对当前判断的置信度
    satisfied_constraints: list[str] # 已满足的显式约束
    failed_constraints: list[str]    # 未满足或证据不足的约束
    view_feedback: str        # 为什么需要更好视角，或当前视角为何可接受
    preferred_view_goal: str  # 给 view planner 的自然语言视角目标
    view_objective: dict      # 给几何视角选择器的结构化目标，如居中、距离、共视
    reason: str               # 可审计的自然语言解释，写入 final_verifier 日志
```
...
Candidate:
  uid: tv:c75ccae926db        # 运行时对象实例 ID，用于 pin target 和 ledger
  label: tv                   # detector / mapper 当前标签
  role: terminal              # terminal 表示该候选可触发最终成功

Evidence:
  - RGB image with candidate bbox                 # 原始图和候选框
  - object crop                                   # bbox 裁剪图，供细粒度确认
  - geometry facts: distance_to_object, bbox_area_ratio, center_offset
                                                  # 距离、尺度和中心度等硬证据
  - relation edges if any                         # 已验证的动态语义边
  - view-control history if any                   # 已尝试视角和 best evidence
...
```text
satisfied: 当前证据是否满足完整原始指令。
semantic_satisfied: 语义是否满足；为 true 但视角差时仍可能 need_better_view。
view_sufficient_for_stop: 当前视角是否足以停止。
decision: planner 应执行的下一步决策。
view_feedback / preferred_view_goal: 给 view-control 的反馈。
reason: 写入日志的可审计解释。
```
...
Subject candidate:
  uid: book:56e406d1ac42      # terminal 候选实例
  label: book                 # 当前 mapper/detector 标签

Anchor candidate:
  uid: cabinet:39f2a38841f1   # anchor 候选实例
  label: cabinet              # detector 标签可与指令概念 shelf 不同
  role: anchor                # anchor 只辅助关系验证，不触发 STOP

Relation:
  on                          # 需要验证的空间/语义关系

Evidence:
  - shared viewpoint images      # 同时观察到 subject 与 anchor 的历史视角
  - check_again image            # 更近或更清晰的复核视角
  - geometry hints               # 距离、相对位置、bbox 投影等几何提示
...
```text
verified: 当前 subject-anchor-relation 三元组是否成立。
confidence: VLM/关系验证器对该关系判断的置信度。
need_better_view: 关系可能成立但当前证据不足时请求更好视角。
reason: 写入 DynamicSemanticEdge / RelationLedger 的可审计解释。
```
...
CameraModel.UNKNOWN:
  ...
  # 缺少完整相机模型时，planner 不能假设全景或 RGB-D 能力。
```
...
```python
RealObservation:
    timestamp              # 观测时间戳，用于同步相机、LiDAR 和里程计
    robot_pose             # 当前机器人位姿，所有 evidence 都应绑定同一 pose
    camera_frames          # 一个或多个相机帧，支持 panorama / pinhole
    pointcloud_ref         # 点云引用，真实大数组由 runtime/cache 管理

DetectionFrame:
    image_ref              # 检测对应的图像引用
    boxes_xyxy             # 2D bbox，进入 crop 和 verifier
    labels                 # detector 原始标签，不在 adapter 层做语义归一化
    confidences            # detector 置信度，只作为候选排序证据
    track_ids              # detector/tracker ID，可辅助对象关联但不一定稳定

SemanticMapSnapshot:
    robot_pose             # snapshot 构建时的机器人位姿
    objects                # SysNav/mapper 输出的对象节点只读快照
    rooms                  # room node 快照，语义房间名可由上层推断
    viewpoints             # 可选视点节点，用于高层搜索和 evidence loop
    frontiers              # 可选探索边界，供 relocation / exploration 使用

NavigationIntent:
    mode                   # 高层语义意图，如 explore / go_to_object / improve_view
    goal_pose              # 希望底层到达的位置；STOP/WAIT 可为空
    target_object_uid      # 当前 terminal 目标实例 ID
    anchor_object_uid      # 关系任务中的 anchor 实例 ID
    relation_edge_id       # 已验证或待维护的动态语义边 ID

MotionGoal:
    mode                   # bridge 可执行的运动模式
    goal_pose              # 发布给 /way_point 或局部规划器的目标位姿
    look_at                # 可选朝向参考，用于证据采集视角
    tolerance              # 到达容差，由底层 controller/status provider 解释

ViewpointGoal:
    pose                   # 需要到达的证据采集视角
    purpose                # improve_view / verify_target / verify_relation
    target_object_uid      # 该视角围绕哪个目标实例采集证据
    evidence_requirements  # 对视角质量、共视、bbox 等证据要求

ViewEvidence:
    image_ref              # 到达视角后采集的图像引用
    pose                   # 采集证据时的机器人/相机位姿
    bbox_xyxy              # 当前目标在图像中的 bbox，可为空
    quality                # 中心度、面积、可见性等视角质量摘要
```
