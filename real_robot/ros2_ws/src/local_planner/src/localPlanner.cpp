...
string cancelTopic = "/local_planner/cancel";
string plannerStatusTopic = "/local_planner/status";
...
int main(int argc, char **argv)
{
  ...
  nh->declare_parameter<string>("cancel_topic", cancelTopic);
  nh->get_parameter("cancel_topic", cancelTopic);
  auto subCancel = nh->create_subscription<std_msgs::msg::Empty>(cancelTopic, 5, cancelHandler);
  nh->declare_parameter<string>("status_topic", plannerStatusTopic);
  nh->get_parameter("status_topic", plannerStatusTopic);
  ...
  auto pubPlannerStatus = nh->create_publisher<std_msgs::msg::String>(plannerStatusTopic, 5);
  nav_msgs::msg::Path path;
  auto publishPlannerStatus = [&](const string & status) {
    std_msgs::msg::String message;
    message.data = status;
    pubPlannerStatus->publish(message);
  };
  ...
  // 中文说明：动作服务需要区分“规划器尚未收到传感器”和“已经确认无可行路径”。
  // 状态 topic 是 motion action 的事实来源，不再通过单点 Path 猜测 blocked。
  publishPlannerStatus("waiting_for_sensor");
  ...
  while (status)
  {
    ...
    if (cancelRequested)
    {
      ...
      // 中文说明：用单点零路径清空 pathFollower 的历史轨迹，同时保留
      // localPlanner 进程运行，等待下一次正式 waypoint，而不是销毁节点。
      path.poses.resize(1);
      ...
      publishPlannerStatus("cancelled");
    }
    ...
    if (newLaserCloud || newTerrainCloud)
    {
      ...
      if (!pathFound)
      {
        ...
        path.header.frame_id = "vehicle";
        pubPath->publish(path);
        publishPlannerStatus("no_feasible_path");
