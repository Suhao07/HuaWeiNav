...
// STRIVE keeps serial/chassis ownership outside the migrated path follower.
...
int main(int argc, char** argv)
{
  ...
  nh->declare_parameter<string>("manual_cmd_topic", "/cmd_vel/manual");
  ...
  string manualCmdTopic;
  nh->get_parameter("manual_cmd_topic", manualCmdTopic);
  ...
  // 核心安全边界：pathFollower 只发布候选自主速度，由唯一 SafetyVelocityMux
  // 负责 watchdog、人工接管、急停和最终 /cmd_vel 输出。
  auto pubSpeed = nh->create_publisher<geometry_msgs::msg::TwistStamped>("/cmd_vel/autonomy", 5);
  auto pubManualSpeed = nh->create_publisher<geometry_msgs::msg::TwistStamped>(manualCmdTopic, 5);
  geometry_msgs::msg::TwistStamped cmd_vel;
  cmd_vel.header.frame_id = "vehicle";
  geometry_msgs::msg::TwistStamped manual_cmd;
  manual_cmd.header.frame_id = "vehicle";
  ...
  while (status) {
    ...
    manual_cmd.twist.linear.x = manualMode ? maxSpeed * joyManualFwd : 0;
    manual_cmd.twist.linear.y = manualMode && omniDirGoalThre > 0 ? maxSpeed / 2.0 * joyManualLeft : 0;
    manual_cmd.twist.angular.z = manualMode ? maxYawRate * PI / 180.0 * joyManualYaw : 0;
    pubManualSpeed->publish(manual_cmd);
