...
def resolve_package_path(path_value, package_dir):
    """Resolve SysNav config/model paths after vendoring into STRIVE."""
    ...
    if candidate.exists():
        return candidate
    # 核心：兼容原 SysNav 配置里的 src/semantic_mapping/semantic_mapping/... 相对路径。
    ...
    def __init__(self, device='cuda'):
        ...
        if not detector_model_path.exists():
            raise FileNotFoundError(
                ...
                "Set detector_model_path or install SysNav detector weights under semantic_mapping/external."
            )
        # 核心：模型类型和路径由参数控制，便于在真机上切换 YOLOE / YOLO-World / TensorRT engine。
        if detector_model_type in ("yolo_world", "yoloworld", "world"):
            self.grounding_model = YOLOWorld(detector_model_path)
        elif detector_model_type in ("yolo", "detect"):
