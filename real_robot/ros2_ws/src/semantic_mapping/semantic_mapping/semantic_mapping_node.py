...
try:
    ...
    # line_profiler 只用于开发 profiling；部署环境缺失时保持节点可运行。
    def profile(fn):
        return fn
...
def resolve_package_path(path_value, package_dir=PACKAGE_DIR):
    """Resolve SysNav source-relative paths after vendoring into STRIVE."""
...
def load_models():
    checkpoint = resolve_package_path(
        os.getenv(
            "SYSNAV_SAM2_CHECKPOINT",
            PACKAGE_DIR / "external" / "sam2" / "checkpoints" / "sam2.1_hiera_base_plus.pt",
        )
    )
    if not checkpoint.exists():
        raise FileNotFoundError(
            f"SAM2 checkpoint not found: {checkpoint}. "
            "Set SYSNAV_SAM2_CHECKPOINT or run the SAM2 checkpoint downloader."
        )
    model_cfg = os.getenv("SYSNAV_SAM2_MODEL_CFG", "configs/sam2.1/sam2.1_hiera_b+.yaml")
    mask_predictor = SAM2ImagePredictor(build_sam2(model_cfg, str(checkpoint)))
